...
public class BookingsFilterContext {
     ...
     */
    public static List<Booking> filterBookings(Long userId, State state, BookingRepository repository, boolean isOwner) {
        BookingsFilterStrategy strategy = strategies.get(state);
        return strategy.filter(userId, repository, isOwner);
    }
}
